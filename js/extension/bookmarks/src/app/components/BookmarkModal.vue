<script setup lang="ts">
    ...
    const filteredTags = computed(() => {
        ...
        return knownTags.value
            ...
            .sort((left, right) => {
                ...
                return (
                    Number(right.searchCapability) - Number(left.searchCapability) ||
    ...
    const canCreateTag = computed(() => {
        ...
        return (
            !knownTags.value.some((tag) => tag.normalizedName === normalizeTag(query)) &&
            !form.tags.some((tag) => normalizeTag(tag) === normalizeTag(query))
        );
    });
            ...
            <DialogContent class="dialog-content bookmark-dialog" @escape-key-down="pending && $event.preventDefault()">
                <header class="dialog-head">
                    <div
                        ...
                        ><DialogDescription class="sr-only">编辑书签信息</DialogDescription></div
                    >
                ...
                <form class="editor-form" @submit.prevent="save">
                    ...
                    <label>
                        ...
                        <SelectRoot v-model="form.parentId" :disabled="pending">
                            <SelectTrigger class="field-select"
                                ><SelectValue placeholder="选择目录" /><ChevronDown :size="14"
                            /></SelectTrigger>
                            <SelectPortal
                                ><SelectContent class="field-select-menu" position="popper" :side-offset="5"
                                    ><SelectViewport>
                                        <SelectItem
                                        ...
                                        >
                                            ...
                                            ><SelectItemIndicator><Check :size="13" /></SelectItemIndicator>
                                ...
                                <div class="tag-editor">
                                    <TagsInputRoot v-model="form.tags" class="tag-token-list" :duplicate="false">
                                        ...
                                        >
                                            ...
                                            <small
                                                v-if="
                                                    normalizeTag(tag) === SEARCH_TAG ||
                                                    normalizeTag(tag) === SEARCH_SITE_TAG
                                                "
                                                >搜索能力</small
                                            >
                                            <TagsInputItemDelete aria-label="移除标签" @click="removeTag(tag)"
                                                ><X :size="12"
                                            /></TagsInputItemDelete>
                                        </TagsInputItem>
                                    </TagsInputRoot>
                            ...
                            <ComboboxPortal>
                                <ComboboxContent class="tag-suggestion-menu" position="popper" :side-offset="4">
                                    <ComboboxViewport>
                                        ...
                                        >
                                            <span>{{ tag.name }}</span
                                            ><small>{{ tag.searchCapability ? '搜索能力' : `${tag.count} 次` }}</small>
                                            <ComboboxItemIndicator><Check :size="13" /></ComboboxItemIndicator>
                                        </ComboboxItem>
                                        <ComboboxItem
                                            v-if="canCreateTag"
                                            class="tag-suggestion create"
                    ...
                    <label v-if="hasSearch"
                        ><span>搜索 URL 模板</span><input v-model="form.searchUrl" :disabled="pending" type="text"
                    /></label>
                    <div v-if="hasSearchSite" class="site-preview"
                        ><span>站点搜索</span
                        ><strong>{{ siteDomain ? `site:${siteDomain}` : '请填写有效 URL' }}</strong></div
                    >
                    <p v-if="validationError || error" class="form-error" role="alert">{{
                        validationError || error
                    }}</p>
                    <footer class="dialog-foot"
                        ><button class="button-secondary" type="button" :disabled="pending" @click="emit('close')"
                            >取消</button
                        ><button class="button-primary" type="submit" :disabled="pending">保存</button></footer
                    >
                </form>
            </DialogContent>
        </DialogPortal>
