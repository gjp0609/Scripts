<script setup lang="ts">
    ...
    const emit = defineEmits<{
        ...
        'search-focus': [];
        'update:engine-menu-open': [value: boolean];
    ...
    function openEngineMenu() {
        ...
        if (!props.engineMenuOpen) {
            emit(
                'engine-active-change',
                Math.max(
                    0,
                    props.engines.findIndex((item) => item.id === props.engine.id),
                ),
            );
        }
        emit('update:engine-menu-open', true);
    }
    ...
    function closeEngineMenu() {
        window.clearTimeout(engineCloseTimer);
        if (props.engineMenuOpen) emit('update:engine-menu-open', false);
    }

    function handleEngineKeydown(event: KeyboardEvent) {
        if (event.key !== 'ArrowDown' && event.key !== 'ArrowUp' && event.key !== 'Enter') return;
        if (!props.engineMenuOpen) openEngineMenu();
