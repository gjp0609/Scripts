...
export function useBookmarkWorkspace() {
    ...
    function upsertFolder(node: BrowserBookmarkNode) {
        ...
        if (!rootChildIds.value.includes(node.id))
            rootChildIds.value.splice(node.index ?? rootChildIds.value.length, 0, node.id);
    }
    ...
    async function handleStorageChange(change: WorkspaceStorageChange) {
        ...
        for (const bookmarkId of change.extraBookmarkIds) {
            ...
            if (!current) continue;
            upsertBookmark(buildBookmarkView(toBrowserNode(current.bookmark), await getExtra(bookmarkId)));
        }
    }
