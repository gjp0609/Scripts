...
export function useSearchCommands(options: {
  search: SearchState;
  ...
  function openBookmark(bookmark: BookmarkView) {
    if (options.mode.value === 'browse' && bookmark.url) void openUrl(bookmark.url);
  }
  ...
  function handleSearchKeydown(event: KeyboardEvent) {
    ...
    if (event.key === 'Enter' && options.search.query.value.trim()) {
      event.preventDefault();
      void openUrl(buildSearchEngineUrl(options.search.currentEngine.value, options.search.query.value.trim()));
    }
  }
