...
export function useSearchState(options: SearchStateOptions) {
    ...
    const quickTargets = computed(() =>
        getQuickSearchTargets(options.folders.value, quickSearch.value?.siteQuery ?? ''),
    );
    const normalSearch = computed(() => !quickSearch.value && !tagSearch.value && Boolean(query.value.trim()));
    const showOverlay = computed(
        () => Boolean(quickSearch.value || tagSearch.value) && !overlaySuppressed.value && !engineMenuActive.value,
    );
