...
export function onBookmarkEvent(handler: (event: BookmarkEvent) => void): () => void {
    ...
    const removedListener = (id: string, removeInfo: BookmarkRemovedInfo) =>
        handler({ type: 'removed', id, removeInfo });
    const movedListener = (id: string, moveInfo: BookmarkMovedInfo) => handler({ type: 'moved', id, moveInfo });
    const reorderedListener = (id: string, reorderInfo: BookmarkReorderedInfo) =>
        handler({ type: 'reordered', id, reorderInfo });
