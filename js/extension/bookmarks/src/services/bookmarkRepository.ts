...
function toBookmarkView(node: BrowserBookmarkNode, extras: Record<string, BookmarkExtra>): BookmarkView {
    const extra = extras[node.id] ?? emptyBookmarkExtra(node.id);
    return {
        ...node,
        extra,
...
export async function loadBookmarkWorkspace(): Promise<{
    rootId: string;
    rootChildIds: string[];
    folders: FolderView[];
}> {
...
export async function saveFolderDetails(input: {
    id?: string;
    parentId: string;
    title: string;
...
export async function saveBookmarkDetails(input: {
    ...
    try {
        ...
        if (!input.id && node) {
            await removeBookmark(node.id).catch(() => {
                rollbackFailed = true;
            });
            await removeExtra(node.id).catch(() => undefined);
        } else if (input.id && originalNode) {
            await updateBookmark(input.id, { title: originalNode.title, url: originalNode.url }).catch(() => {
                rollbackFailed = true;
            });
            if (originalNode.parentId != null || originalNode.index != null) {
                await moveNode(input.id, { parentId: originalNode.parentId, index: originalNode.index }).catch(() => {
                    rollbackFailed = true;
                });
            }
            if (originalExtra) {
                await restoreExtra(originalExtra).catch(() => {
                    rollbackFailed = true;
                });
            } else {
                await removeExtra(input.id).catch(() => {
                    rollbackFailed = true;
                });
            }
        }
