...
export async function getPreferences(): Promise<UiPreferences> {
    const data = (await browser.storage.local.get([SEARCH_ENGINE_KEY, COLLAPSED_FOLDERS_KEY])) as Record<
        string,
        unknown
    >;
    return {
        searchEngine:
            typeof data[SEARCH_ENGINE_KEY] === 'string' ? data[SEARCH_ENGINE_KEY] : defaultPreferences.searchEngine,
...
export async function removeBackupNodeMappings(entries: readonly BackupNodeMappingEntry[]): Promise<void> {
    ...
    const keys = entries.map(
        ({ originId, sourceId }) => `${backupNodeMapPrefix(originId)}${encodeURIComponent(sourceId)}`,
    );
