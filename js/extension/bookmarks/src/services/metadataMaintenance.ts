...
export async function repairExtensionData(): Promise<MetadataRepairReport> {
  ...
  const safeExtraPatch = new Map(
    [...plan.extraPatch].filter(([bookmarkId]) => JSON.stringify(latestExtras[bookmarkId]) === JSON.stringify(extras[bookmarkId]))
  );
  const latestMappingTargets = new Map(
    latestMappings.map((mapping) => [`${mapping.originId}\u0000${mapping.sourceId}`, mapping.targetId])
  );
  const safeStaleMappings = plan.staleMappings.filter((mapping) => (
    latestMappingTargets.get(`${mapping.originId}\u0000${mapping.sourceId}`) === mapping.targetId
  ));
  const canRepairPreferences = plan.report.preferencesRepaired
    && JSON.stringify(latestPreferences) === JSON.stringify(preferences);
