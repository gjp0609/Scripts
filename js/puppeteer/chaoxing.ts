...
async function waitPage(url: string): Promise<puppeteer.Page> {
                ...
                if (studyPage) {
                    ...
                    let result = await studyPage.evaluate(
                        (answerIndex, lastOptionLength) => {
                             ...
                             * 获得指定数组的所有组合
                             */
                            function arrayCombine(targetArr: number[] = [], count = 1) {
                                const resultArrs = [];
                                // 所有组合的 01 排列
                             ...
                             */
                            function getFlagArrs(m: number, n = 1) {
                                if (n < 1 || m < n) return [];
                            ...
                            function getAnswers(length: number) {
                                let arr: number[] = [];
                            ...
                            if (optionLength > 1) {
                                ...
                                for (let option of optionList) {
                                    (option.querySelector('input[type=checkbox]') as HTMLInputElement).checked = false;
                                }
                                for (let option of answer) {
                                    (optionList[option] as HTMLOptionElement).click();
                                }
                                answerIndex++;
                        ...
                        if (tips) {
                            ...
                            for (let i = 0; i < chapterList.length; i++) {
                                if (chapterList[i].className.indexOf('posCatalog_active') >= 0) {
                                    console.log(new Date(), '当前章节:', (chapterList[i] as HTMLDivElement).innerText);
                                    let nextChapter = chapterList[i + 1].querySelector('span.posCatalog_name');
                                    if ((nextChapter as HTMLSpanElement).title === '章节测验') {
                                    ...
                                    (nextChapter as HTMLSpanElement).click();
                                    break;
                                }
                            }
                        } else {
